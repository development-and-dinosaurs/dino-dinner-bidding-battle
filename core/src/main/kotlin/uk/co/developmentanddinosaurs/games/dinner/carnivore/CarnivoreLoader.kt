 ...
 *
 * This CarnivoreLoader is designed to load any [CraftyCodeCarnivore] classes that are present in
 * the execution directory where we're running the game. These classes need to be packaged up in a
 * Jar file and present in the `carnivores` directory.
 *
...
class CarnivoreLoader(private val directory: File) {
  ...
  fun loadCarnivores(): List<CraftyCodeCarnivore> {
    val carnivoreJars =
        File(directory, "carnivores").listFiles(FileFilter { it.extension == "jar" })
            ?: emptyArray()
    val carnivores =
        carnivoreJars.map { jar ->
          ...
          JarFile(jar)
              ...
              .filter { clazz -> clazz.interfaces.contains(CraftyCodeCarnivore::class.java) }
              .map { clazz ->
    ...
    return if (carnivores.size > 6) carnivores else (carnivores + fairCarnivores).take(6)
  }
}
